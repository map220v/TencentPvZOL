package asgui.serialization.graphics
{
   ...
   public class JPEGEncoder implements IImageEncoder
   {
      ...
      private function internalEncode(param1:Object, param2:int, param3:int, param4:Boolean = true) : ByteArray
      {
         ...
         if(this.bytepos >= 0)
         {
            (_loc12_ = new BitString()).len = this.bytepos + 1;
      ...
      private function RGB2YUV(param1:BitmapData, param2:ByteArray, param3:int, param4:int, param5:int, param6:int) : void
      {
         ...
         while(_loc8_ < 8)
         {
            if((_loc9_ = param4 + _loc8_) >= param6)
            {
               _loc9_ = param6 - 1;
            }
            _loc10_ = 0;
            while(_loc10_ < 8)
            {
               if((_loc11_ = param3 + _loc10_) >= param5)
               {
                  _loc11_ = param5 - 1;
               }
               if(param1)
               ...
               _loc13_ = Number(_loc12_ >> 16 & 255);
               _loc14_ = Number(_loc12_ >> 8 & 255);
               _loc15_ = Number(_loc12_ & 255);
