package com.qq.modules.entryIcon.policy
{
   ...
   public class LimitBeginPolicy extends BasePolicy
   {
      ...
      public function LimitBeginPolicy()
      {
         super(PolicyType.TYPE_LIMIT_BEGIN);
      ...
      private function checkIsBegin(param1:ActivityCommonInfo) : Boolean
      {
         if(param1 && param1.limitCondition)
         {
            return DateUtils.getInstance().getInterval(param1.limitCondition.uBeginTime) <= 0;
         }
         return false;
      }
      ...
      private function checkAll() : void
      {
         ...
         {
            if(this.checkIsBegin(_loc2_))
            {
               _loc1_.push(_loc2_);
               EntryIconCmd.changeIconEffect(_loc2_.iconID,true);
            }
         }
         if(_loc1_.length > 0)
         {
