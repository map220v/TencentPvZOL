package com.qq.modules.collectWord.model
{
   ...
   import com.qq.modules.collectWord.command.CollectWordCmd;
   import com.qq.modules.collectWord.model.vo.CollectWordExchangeVO;
   import com.qq.modules.main.command.CommonCmd;
   import tencent.io.service.events.SocketServiceEvent;
   
   public class CollectWordProxy extends BasicProxy
   {
      ...
      private var m_words:Vector.<Dto_IdNum>;
      
      private var m_vecExchange:Vector.<CollectWordExchangeVO>;
      ...
      public function CollectWordProxy()
      {
         super();
      }
      ...
      public function get words() : Vector.<Dto_IdNum>
      {
         return this.m_words;
      }
      ...
      {
         super.init(param1);
         this.m_vecExchange = new Vector.<CollectWordExchangeVO>();
      }
      ...
      public function get totalTicketCost() : uint
      {
         var _loc1_:int = 0;
         var _loc2_:int = 0;
         while(_loc2_ < this.m_words.length)
         {
            if(this.m_words[_loc2_].num <= 0)
            {
               _loc1_++;
            }
            _loc2_++;
      ...
      public function get exchangeList() : Vector.<CollectWordExchangeVO>
      {
      ...
      private function onFlexibleActivity_1005(param1:SocketServiceEvent) : void
      {
         ...
         if(_loc2_.activityid == CollectWordConst.ACTIVITY_ID)
         {
            this.m_acitivityInfo = this.activityProxy.getGroupByViewType(ActivityGlobal.VIEW_TYPE_COLLECT_WORD)[0];
            this.m_totalScore = _loc2_.credit;
            this.m_playerTicketCost = _loc2_.lackmembercost;
            if(this.m_words == null)
            {
               this.m_words = new Vector.<Dto_IdNum>();
               _loc4_ = 0;
               while(_loc4_ < _loc2_.activitymembernum)
               {
                  (_loc5_ = new Dto_IdNum()).id = _loc4_ + 1;
                  _loc5_.num = 0;
                  this.m_words.push(_loc5_);
                  _loc4_++;
               }
            }
            ...
            {
               _loc4_ = 0;
               while(_loc4_ < this.m_words.length)
               {
                  if(this.m_words[_loc4_].id == _loc5_.id)
                  {
                     this.m_words[_loc4_].num = _loc5_.num;
                  }
                  _loc4_++;
               }
            }
            ...
            {
               this.m_vecExchange.push(new CollectWordExchangeVO(_loc3_));
            }
            this.dispatch(new CommonEvent(CollectWordConst.EVENT_UPDATE));
         }
      }
      ...
      private function onCmd_DragonBoat_Summon_SC(param1:SocketServiceEvent) : void
      {
         ...
         if(_loc2_.activityid == CollectWordConst.ACTIVITY_ID)
         {
            CollectWordCmd.getInfo();
            this.dispatch(new CommonEvent(CollectWordConst.EVENT_SUMMON,_loc2_.itemids));
         }
      }
      ...
      private function onCmd_DragonBoat_SetOut_SC(param1:SocketServiceEvent) : void
      {
         ...
         if(_loc2_.activityid == CollectWordConst.ACTIVITY_ID)
         {
            ...
            CollectWordCmd.getInfo();
            this.dispatch(new CommonEvent(CollectWordConst.EVENT_SETOUT));
         }
      }
   }
}
